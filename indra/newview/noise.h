...
static void init(void)
{
    ...
    /* <TommyTheTerrible> Removing seeded random and replacing with numbers generated by Microsoft's rand
    srand(42);
    </TommyTheTerribe> */
    int i, j, k;
    ...
    for (i = 0 ; i < B ; i++) {
        ...
        g1[i] = (F32)((175 % (B + B)) - B) / B; // TommyTheTerrible - Windows Seeded Random First Run: 175

        for (j = 0 ; j < 2 ; j++)
            g2[i][j] = (F32)((400 % (B + B)) - B) / B; // TommyTheTerrible - Windows Seeded Random Second Run: 400
        normalize2(g2[i]);

        for (j = 0 ; j < 3 ; j++)
            g3[i][j] = (F32)((17869 % (B + B)) - B) / B; // TommyTheTerrible - Windows Seeded Random Third Run: 17869
        normalize3(g3[i]);
    }

    while (--i) {
        k = p[i];
        p[i] = p[j = 30056 % B]; // TommyTheTerrible - Windows Seeded Random Fourth Run: 30056
        p[j] = k;
    }
